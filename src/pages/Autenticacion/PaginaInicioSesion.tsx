...
import {
  esquemaInicioSesion,
  type DatosFormularioInicioSesion,
  esquemaNuevaContrasena,
  type DatosFormularioNuevaContrasena,
} from "@maximilian/schemas";
import { servicioAutenticacion } from "@maximilian/services/autenticacion.service";
import { traducirErrorAutenticacion } from "@maximilian/shared/utils/errores-autenticacion";
import { consumirMensajeSesion } from "@maximilian/services/sesion.service";

export default function PaginaInicioSesion() {
  const [mostrarContrasena, setShowPassword] = useState(false);
  const [mostrarConfirmacionContrasena, setShowConfirmPassword] = useState(false);
  ...
  } = useForm<DatosFormularioInicioSesion>({
  ...
  useEffect(() => {
    ...
    const datosGuardados = Cookies.get(REMEMBER_ME_COOKIE);
    if (datosGuardados) {
      try {
        const { username, password } = JSON.parse(datosGuardados);
        if (username) setLoginValue("username", username);
        if (password) setLoginValue("password", password);
        setLoginValue("recordarme", true);
      } catch (e) {
  ...
  } = useForm<DatosFormularioNuevaContrasena>({
  ...
  const onLogin = async (data: DatosFormularioInicioSesion) => {
    setIsLoading(true);
    setAuthError(null);
    try {
      const response = await servicioAutenticacion.login(data);
      ...
      setAuthError(traducirErrorAutenticacion(err));
    } finally {
      setIsLoading(false);
    }
  };

  const onNewPassword = async (data: DatosFormularioNuevaContrasena) => {
    setIsLoading(true);
    setAuthError(null);
    try {
      const response = await servicioAutenticacion.confirmNewPassword(data.newPassword);
      if (response.isSignedIn) {
        navigate("/seleccionar-rol");
      } else {
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <button
                  type="button"
                  onClick={() => setShowPassword(!mostrarContrasena)}
                  disabled={isLoading}
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <button
                  type="button"
                  onClick={() => setShowConfirmPassword(!mostrarConfirmacionContrasena)}
                  disabled={isLoading}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-brand-black transition-colors disabled:opacity-50 cursor-pointer"
                >
        ...
        {!isNewPasswordRequired && (
          <div className="mt-10 text-center">
            <p className="text-xs text-gray-500">
              ...
              <button
                type="button"
                onClick={() => navigate("/olvide-contrasena", { state: { username: watch("username") } })}
