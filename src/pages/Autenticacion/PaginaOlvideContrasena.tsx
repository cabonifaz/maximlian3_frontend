...
import { 
  ...
  type DatosFormularioOlvideContrasena,
  esquemaRestablecerContrasena,
  type DatosFormularioRestablecerContrasena
...
export default function PaginaOlvideContrasena() {
  ...
  } = useForm<DatosFormularioOlvideContrasena>({
  ...
  } = useForm<DatosFormularioRestablecerContrasena>({
  ...
  const onRequestCode = async (data: DatosFormularioOlvideContrasena) => {
    setIsLoading(true);
    setAuthError(null);
    try {
      const output = await servicioAutenticacion.resetPassword(data.username);
  ...
  const onResetPassword = async (data: DatosFormularioRestablecerContrasena) => {
    setIsLoading(true);
    setAuthError(null);
    setSuccessMessage(null);
    try {
      await servicioAutenticacion.confirmPasswordReset(username, data.code, data.newPassword);
      // Success: redirect to login
      navigate("/iniciar-sesion", { state: { message: "Contraseña actualizada exitosamente. Inicia sesión con tu nueva contraseña." } });
    } catch (err: unknown) {
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-brand-black transition-colors disabled:opacity-50"
                >
                  {mostrarConfirmacionContrasena ? <EyeOff size={20} /> : <Eye size={20} />}
                </button>
              </div>
