...
export function Header({ role: initialRole }: HeaderProps) {
  ...
  const userName = userSession?.nombres || "Usuario";
  const userInitials = getInitials(userName);
  const availableRoles = userSession?.roles || [];
    ...
    <header className="h-16 bg-brand-white border-b border-gray-100 px-8 flex items-center justify-end gap-6 sticky top-0 z-50 shrink-0">
      <div className="flex items-center gap-4 border-l pl-6 border-gray-100 h-10">
        <div className="text-right hidden sm:block">
          ...
          <p className="text-[10px] text-gray-500 capitalize">{selectedRole.toLowerCase()}</p>
        </div>
            ...
            <>
              ...
              <div className="absolute right-0 mt-2 w-64 bg-brand-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-20 animate-in fade-in zoom-in-95 duration-100">
                <div className="px-4 py-2 border-b border-gray-50 mb-1">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Cambiar rol</p>
                </div>
                ...
                <div className="max-h-48 overflow-y-auto">
                  {availableRoles.map((role) => (
                    ...
                    >
                      <span className={`capitalize ${selectedRole === role.rol ? "font-bold text-brand-wine" : ""}`}>
