...
import type { GrupoFacturacionAnaliticaDashboard } from "@maximilian/shared/types/dashboard.type";
  ...
  return (
    <div className="flex items-center gap-5">
      ...
      <ul className="flex-1 space-y-2">
        {segmentos.map(({ grupo, color, porcentaje }) => (
...
function calcularSegmentosTorta(
  ...
  return grupos.reduce<
    Array<{ grupo: GrupoFacturacionAnaliticaDashboard; color: string; inicio: number; fin: number; porcentaje: number }>
  >((segmentos, grupo, indice) => {
    const inicio = segmentos.at(-1)?.fin ?? 0;
    const porcentaje = (grupo.montoFacturado / total) * 100;

    return [
      ...segmentos,
      { grupo, color: obtenerColor(grupo, indice), inicio, fin: inicio + porcentaje, porcentaje },
    ];
  }, []);
}
