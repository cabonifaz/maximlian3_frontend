...
import { COLUMNAS_TABLA_DESEMPENO_COLABORADORES_DASHBOARD } from "@maximilian/shared/constants/components/gerente/desempeno-colaboradores-dashboard.constants";
...
export function CustomTablaDesempenoColaboradoresGerente({
  ...
  return (
    <CustomTabla
      ...
      paginaActual={1}
      totalPages={1}
      totalRecords={resumenColaboradores.length}
      onPageChange={() => undefined}
      entityLabel="colaboradores"
      cantidadFilasVisibles={Math.min(6, Math.max(resumenColaboradores.length, 1))}
      renderRow={(colaborador) => (
        <>
          <td className="px-6 py-4">
            <div className="flex items-center gap-3">
              <span
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-bold"
                style={{ color: colaborador.colorLetra, backgroundColor: colaborador.colorFondo }}
              >
