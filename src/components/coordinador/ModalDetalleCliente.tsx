...
function opcionesIncluyenValor(options: EntradaTablaMaestra[] | undefined, valor: number | undefined) {
  if (!options || typeof valor !== "number" || !Number.isFinite(valor) || valor <= 0) return true;
  return options.some((option) => option.num1 === valor);
}

function filtrarValoresValidos(options: EntradaTablaMaestra[] | undefined, valores: number[] | undefined) {
...
export function ModalDetalleCliente({
  ...
  const {
    ...
    queryFn: () => (clientId ? servicioCliente.getById(clientId) : null),
  ...
  } = useForm<DatosFormularioDetalleCliente>({
    resolver: zodResolver(esquemaDetalleCliente),
    mode: "onTouched",
  });
  ...
  const updateInfoMutation = useMutation({
    mutationFn: servicioCliente.update,
    onSuccess: () => {
  ...
  const deleteTarifarioMutation = useMutation({
    mutationFn: servicioCliente.deleteTarifario,
    onSuccess: () => {
  ...
  const createContactoMutation = useMutation({
    mutationFn: servicioCliente.createContacto,
  ...
  const updateContactoMutation = useMutation({
    mutationFn: servicioCliente.updateContacto,
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["contactos", client?.idCliente] });
      if (variables.idTipoContacto === 0) {
        queryClient.invalidateQueries({ queryKey: ["masterTable", TablaMaestraId.TIPO_CONTACTO] });
  ...
  const deleteContactoMutation = useMutation({
    mutationFn: servicioCliente.deleteContacto,
    onSuccess: () => {
  ...
  const { data: formatoInformeData } = useQuery({
    queryKey: ["masterTable", TablaMaestraId.TIPO_FORMATO_INFORME],
    queryFn: () => servicioTablaMaestra.list(TablaMaestraId.TIPO_FORMATO_INFORME),
    enabled: isOpen,
    staleTime: Infinity,
  });
  ...
  const debouncedTarifarioSearch = useRetardo(tarifarioSearch);
  ...
  const { data: tarifarioData, isLoading: tarifarioLoading } = useQuery({
    ...
    queryFn: () => servicioCliente.listTarifario({
  ...
  const { data: contactosData, isLoading: contactosLoading } = useQuery({
    ...
    queryFn: () => servicioCliente.listContactos({
      idCliente: client!.idCliente,
  ...
  useEffect(() => {
    ...
    const validarSelector = (
      campo: keyof DatosFormularioDetalleCliente,
                ...
                >
                  ...
                  <CustomSelectorBuscable
                    ...
                    obtenerEtiquetaOpcion={(opcion) =>
                      obtenerEtiquetaTipoRegistroTributario(opcion, tipoPersonaData)
                    }
                    onChange={(val) =>
                  ...
                  <div className="space-y-2">
                    <CustomLabel required={tieneTipoRegistroTributarioSeleccionado}>Registro Tributario</CustomLabel>
                    <input
                    ...
                    <div className="flex gap-2">
                      ...
                      <CustomButton
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const detail = await servicioCliente.getTarifarioById({
                    ...
                    <div className="flex gap-2">
                      ...
                      <CustomButton
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const detail = await servicioCliente.getContactoById({
