                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {infoErrors.numRegistroTributario && (
                      <p className="text-xs text-red-500">
                        {infoErrors.numRegistroTributario.message}
                      </p>
                    )}
                  </div>
                  ...
                  <CustomSelectorBuscable
                    ...
                    onChange={(val) =>
                      setInfoValue("idiomaFacturacion", val, {
                        shouldValidate: true,
                      })
                    }
                  ...
                  <MultiCustomSelectorBuscable
                    ...
                    onChange={(val) =>
                      setInfoValue("formatoInforme", val, {
                        shouldValidate: true,
                      })
                    }
                  ...
                  <CustomSelectorBuscable
                    ...
                    onChange={(val) =>
                      setInfoValue("plantillaInforme", val, {
                        shouldValidate: true,
                      })
                    }
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <CustomButton
                      ...
                      disabled={selectedContactIndex === null}
                      onClick={() => {
                        const next = addedContacts.filter(
                          (_, idx) => idx !== selectedContactIndex,
                        );
                        const newTotal = Math.ceil(
                          next.length / CONTACTS_PER_PAGE,
                        );
                        if (contactsPag > newTotal && newTotal > 0)
                          setContactsPag(newTotal);
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {(() => {
                        ...
                        const paged = filtered.slice(
                          (contactsPag - 1) * CONTACTS_PER_PAGE,
                          contactsPag * CONTACTS_PER_PAGE,
                        );
                          ...
                          <>
                            {paged.map((contact) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-3 text-gray-600">
                                  {contact.tipoContactoLabel}
                                </td>
                                <td className="px-4 py-3 text-gray-600">
                                  {contact.areaTrabajoLabel}
                                </td>
                              ...
                              <tr>
                                <td colSpan={7}>
                                  <div className="flex items-center justify-end gap-2 px-2 py-2">
                                    <button
                                      disabled={contactsPag === 1}
                                      onClick={() =>
                                        setContactsPag((p) => p - 1)
                                      }
                                      className="px-2 py-1 text-sm border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors cursor-pointer"
                                    >
                                      ‹
                                    </button>
                                    <span className="text-sm">
                                      {contactsPag} / {totalPages}
                                    </span>
                                    <button
                                      disabled={contactsPag === totalPages}
                                      onClick={() =>
                                        setContactsPag((p) => p + 1)
                                      }
                                      className="px-2 py-1 text-sm border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors cursor-pointer"
            ...
            <div>
              {activeTab !== "info" && (
                <CustomButton
                  variant="secondary"
                  onClick={() =>
                    setActiveTab(activeTab === "rates" ? "info" : "rates")
                  }
                >
                  Anterior
                </CustomButton>
            ...
            <div className="flex gap-3">
              {activeTab !== "contacts" && (
                <CustomButton
                  variant="secondary"
                  onClick={() =>
                    setActiveTab(activeTab === "info" ? "rates" : "contacts")
                  }
                >
                  Siguiente
                </CustomButton>
        ...
        defaultValues={
          selectedContactIndex !== null
            ? {
                ...
                tipoContacto:
                  addedContacts[selectedContactIndex].tipoContactoId,
                codigoContacto:
                  addedContacts[selectedContactIndex].codigoContacto,
